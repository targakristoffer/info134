...
for(var i =0; i < mydata.length; i++){
  ...
  var pos = {lat:60.395946, lng:5.325745};
...
console.log(arr.length + " HEI");
  var result = [];
  var teller = 0;
  for(var a = 0; a < arr.length; a++){
      console.log("ny test "+arr[a]);
  }
  for(var i = 0; i < arr.length; i++){
    for(var j = i+1; j < arr.length; j++){
      if(arr[i] == arr[j]){
        result[teller] = arr[i];
        console.log("FUNNET : "+ result[teller]);
        teller = teller + 1;
        i = i + 1;
        j = i + 1;
      }
  ...
  for(var a = 0; a < new_result.length; a++){
    console.log("tredje test "+new_result[a]);
  }
  return new_result;
